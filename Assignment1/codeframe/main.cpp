...
//    #ifdef DEBUG
    ...
    std::cout <<    " with first corners ";
    std::cout << parameters.parallel.firstCorner[0] << ", ";
    std::cout << parameters.parallel.firstCorner[1] << " and ";
    std::cout << parameters.parallel.firstCorner[2];
...
//    #endif
    ...
    while (time < parameters.simulation.finalTime){
      ...
      if( abs(time - vtkTime) < DBL_EPSILON || time > vtkTime  ){
          simulation->plotVTK(timeSteps);
          vtkTime += timeVtkOut;
      }
      timeSteps++;
